...
const SRScreen = (props) => {
    ...
    useEffect(() => {
        if (get_siebelReport && reportDataClick) {
            ...
            var path = DownloadDirectoryPath + '/' + Inumber + "_SibleReport.pdf";
                    ...
                    writeFile(path, parsedSiebeleport.FileBuffer, 'base64')
                        .then((success) => {
                            ...
                            FileViewer.open(`file://${path}`, {
                                ...
                                type: 'application/pdf', // MUST match file type
                              })
                                .then(() => {
                                    console.log('FILE Open!');
                                })
                                .catch(e => {
                                    console.log('FILE Open failed!::' + e);
                                    if (e?.message?.includes('No app')) {
                                        Alert.alert(
                                          'No PDF app found',
...
//                     ]
//                 ).then(async (result) => {
// console.log('result_file',result );
...
//                 });
    ...
    async function saveAndOpenFile(base64Data, fileName) {
        try {
          const path =
            Platform.OS === 'android'
              ? path
          ...
          // 🔹 No permission required (Scoped Storage compliant)
